  ...
  return (
    ...
    <div className="relative overflow-hidden border-b border-[var(--color-border)]">
      ...
      >
        ...
        {/* ── Row 2: Time block — always its own row so it's visible on mobile ── */}
        {hasTimeBlock && !task.is_completed && (
          <div className="flex items-center gap-1.5 mt-1.5 pl-[calc(18px+1.75rem+0.75rem)] flex-wrap">
            <span className="text-[var(--color-text-ghost)] text-sm font-light">[</span>
            {task.start_time && (
              <span className="px-2 py-0.5 bg-[#FDF8EE] text-[#B8934A] border border-[#EADDBE] rounded-md text-xs font-semibold">
            ...
            <span className="text-[var(--color-text-ghost)] text-sm font-light">]</span>
            {task.start_time && task.end_time && (
              <span className="px-2 py-0.5 bg-[#eaddbe]/30 text-[#8b6f3b] text-xs font-bold rounded-md">
